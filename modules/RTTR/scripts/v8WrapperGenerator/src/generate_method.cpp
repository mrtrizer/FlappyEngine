...
std::string generateMethodBody(std::string methodName, std::string className, const std::vector<CXXMethodDecl*> methods) {
    ...
    for (auto methodDecl : methods) {
        ...
        if (methodDecl->isConst()) {
            methodConditionBlock
                     << ".method(\"" << methodName << "\", select_const(&"
                     << className << "::" << methodName << "))" << std::endl;
        } else {
            methodConditionBlock
                     << ".method(\"" << methodName << "\", select_overload<"
                     << resultTypeName << "(" << args << ")>(&"
                     << className << "::" << methodName << "))" << std::endl;
        }
    }
    ...
    if (!classDecl->isAbstract()) {
        for (auto methodDecl : methods) {
            if (methodDecl->isUserProvided())
                constructorBody << ".constructor<" << generateArgs(methodDecl) << ">()" << std::endl;
        }
        // default constructor
        if (classDecl->hasDefaultConstructor())
            constructorBody << ".constructor<>()" << std::endl;
    }
...
GeneratedMethods processMethods(const CXXRecordDecl* classDecl, const std::string& className) {
    ...
    for (auto methodPair : methods) {
        ...
        if (methodName == className) {
            methodBodies << generateConstructorBody(overloadedMethods, classDecl);
        } else {
